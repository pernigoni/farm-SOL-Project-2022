...
for i in 100 150 19 116 2 1 117 3 5 17 4 16 19 8 10 111 12 13 14 15 18 20; do
    ./bin/generafile file$i.dat $(($i*11 + $j*117)) > /dev/null
    j=$(($j+3))
done
...
./bin/farm -n 2 -q 1 file* -d testdir | grep "file*" | awk '{print $1,$2}' | diff - expected.txt
if [[ $? != 0 ]]; then
    echo "test1 failed"
else
    echo "test1 passed"
...
./bin/farm -d testdir -n 8 -q 16 file* | grep "file*" | awk '{print $1,$2}' | diff - expected.txt
if [[ $? != 0 ]]; then
    echo "test2 failed"
else
    echo "test2 passed"
...
# 
./bin/farm -n 1 -d testdir -q 1 -t 1000 file* 2>&1 > /dev/null &
pid=$!
sleep 5
pkill farm
wait $pid
...
# exit status 1.
#
valgrind --error-exitcode=1 --log-file=/dev/null ./bin/farm -d testdir file* 2>&1 > /dev/null
if [[ $? != 0 ]]; then
    echo "test4 failed"
else
    echo "test4 passed"
...
valgrind --leak-check=full --error-exitcode=1 --log-file=/dev/null ./bin/farm file* -d testdir 2>&1 > /dev/null
if [[ $? != 0 ]]; then
    echo "test5 failed"
else
    echo "test5 passed"
